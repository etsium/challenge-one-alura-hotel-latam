...
public class RegistroHuesped extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RegistroHuesped frame = new RegistroHuesped(new Reserva(new Date(), new Date(), "Tarjeta de Crédito"));
	...
	public RegistroHuesped(Reserva reserva) {
		...
		txtNacionalidad.setModel(new DefaultComboBoxModel<String>(new String[] { "Afghanistan – Afeganistão",
				...
				"Zimbabwean – zimbabueano" }));		txtNacionalidad.setBounds(576, 350, 255, 33);
		...
		btnGuardar.addActionListener(new ActionListener() {
				...
				if(huesped != null) {
					...
					if(errores.isEmpty()) {
						...
						try {
							...
							huespedController.guardar(reserva.getId(), huesped);
							completado = true;
						}catch (Exception ex) {
						...
						if(completado) {
							...
							MostrarMensajeError("Ocurrio un error al intentar Guardar los datos.");
						}
			...
			private ArrayList<String> validarCampos() {
				...
				if(txtNombre.getText().equals("")) {
					errores.add("Falta rellenar el nombre");
				}
				if(txtApellido.getText().equals("")) {
					errores.add("Falta rellenar el apellido");
				}
				if(txtFechaN.getDate() == null ) {
					errores.add("Falta rellenar la fecha de nacimineto");
				}
				if(txtTelefono.getText().equals("")) {
					errores.add("Falta rellenar el telefono");
				}
				return errores;
			}
